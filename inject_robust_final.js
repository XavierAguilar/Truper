...
// 1. Re-apply Cuerda Fina Color (Option F2)
html = html.replace('#13a650', '#1cd468');

// 2. Inject CSS
const cssSearch = '.bktpl-btn-print:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(16,185,129,0.5); }';
const cssInject = `
...
`;
html = html.replace(cssSearch, cssInject);

// 3. Inject HTML UI
const htmlSearch = '<div class="bktpl-preview-canvas" id="bktplCanvasWrapper">';
const i1 = html.indexOf(htmlSearch);
if(i1 !== -1) {
    const innerCanvas = html.indexOf('<div id="bktplLiveCanvas"></div>', i1);
    const closeDiv = html.indexOf('</div>', innerCanvas);
    const i2 = html.indexOf('</div>', closeDiv + 1) + 6; // closes bktplCanvasWrapper

    const oldBlock = html.substring(i1, i2);
    const injectedHtml = oldBlock + `
...
// Find selectTemplate function and the JsBarcode setTimeout
const selectTemplateMarker = 'this.activeTemplateId = id;';
const selectIdx = html.indexOf(selectTemplateMarker);
if(selectIdx !== -1) {
    const timeoutIdx = html.indexOf('setTimeout(() => {', selectIdx);
    const endTimeoutIdx = html.indexOf('}, 50);', timeoutIdx);
    if(timeoutIdx !== -1 && endTimeoutIdx !== -1) {
        const barcodeBlock = html.substring(timeoutIdx, endTimeoutIdx + 7);
        const hook = '\\n                        this.renderColorPicker();\\n                        this.applyColorOverrides();';
        const newBarcodeBlock = barcodeBlock.replace('});', '});' + hook);
        html = html.replace(barcodeBlock, newBarcodeBlock);
    }
}

// 6. Inject hook into triggerPrint
const printMarker = "printArea.innerHTML = '<style>@media print { .bt-print-container > div { flex: 0 0 ' + tpl.width + ' !important; min-width: 0 !important; max-width: ' + tpl.width + ' !important; overflow: hidden !important; } .bt-print-container svg { max-width: 100% !important; height: auto !important; } } ' + tpl.css + '</style>';";
if(html.includes(printMarker)) {
    html = html.replace(printMarker, printMarker + '\\n                      this.applyColorOverrides();');
}
