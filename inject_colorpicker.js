...
let cssInject = `
        ...
        /* COLOR PICKER STYLES */
        .bktpl-color-picker { background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; margin: 15px 24px; padding: 12px; }
        .bktpl-color-picker .cp-header { font-size: 13px; font-weight: 700; color: #475569; margin-bottom: 15px; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; display: flex; justify-content: space-between; align-items: center; }
        .bktpl-color-picker .cp-header button { background: none; border: none; cursor: pointer; color: #ef4444; font-size: 11px; font-weight: bold; }
        .bktpl-color-picker .cp-header button:hover { text-decoration: underline; }
...
// 2. Inject HTML
let htmlInject = `
                <div class="bktpl-preview-canvas" id="bktplCanvasWrapper">
                    <div id="bktplLiveCanvas"></div>
                </div>
                
                <div class="bktpl-color-picker" id="bktplColorPicker" style="display:none;">
                    <div class="cp-header">
                        <span><i class="fas fa-palette"></i> Personalizar Plantilla</span>
                        <button onclick="window.bkGallery.resetColors()"><i class="fas fa-undo"></i> Restaurar Defecto</button>
                    </div>
                    <div class="cp-controls" id="bktplColorControls"></div>
                </div>
                <!-- Dynamic Override Style -->
                <style id="bktplDynamicCSS"></style>
`;
html = html.replace(/<div class="bktpl-preview-canvas" id="bktplCanvasWrapper">\s*<div id="bktplLiveCanvas"><\/div>\s*<\/div>/, htmlInject);

// 3. Update activeTemplate logic to call render overrides
        ...
        window.bkGallery = {
            ...
            saveColor(tplId, targetIndex, hexVal) {
                try {
                    ...
                    // Actualizar el input hex visualmente
            ...
            applyColorOverrides() {
                ...
                // Add important overrides for live preview and print area!
                ...
                // Re-inject dynamic print styles if printing
...
// 4. Hook applyColorOverrides and renderColorPicker inside selectTemplate
let initHook = `
                    setTimeout(() => {
                        if(window.JsBarcode) {
...
html = html.replace(/setTimeout\(\(\) => \{[\s\S]*?\}, 50\);/, initHook);

// 5. Hook applyColorOverrides into triggerPrint
let printHook = `
...
html = html.replace("printArea.innerHTML = '<style>@media print { .bt-print-container > div { flex: 0 0 ' + tpl.width + ' !important; min-width: 0 !important; max-width: ' + tpl.width + ' !important; overflow: hidden !important; } .bt-print-container svg { max-width: 100% !important; height: auto !important; } } ' + tpl.css + '</style>';", printHook);

fs.writeFileSync('index.html', html, 'utf8');
console.log("Color picker injected via script successfully.");
